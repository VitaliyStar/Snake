...
void SRecord::as_string(char* buffer) {
    //printf(buffer, recordFormatStr, name, rating, length, game_time, ctime_s(buffer,80, & date));  //uncomment
    //printf(buffer, recordFormatStr, name, rating, length, game_time);  //uncomment
}
...
void CGame::top10(bool after_game) {
    ...
    if (after_game) {
        ...
        scr.pos(5, 16);
        //printf(recordFormatStr, "Your result", rating, snake.size(), duration_game, ctime_s(buf , sizeof(buf), & date)); //uncomment
    }
    ...
    if (rating > ttop10[9].rating) {    // если рейтинг игры больше, чем меньший из 10 лучших...
        ...
        clearkeys();
        //OemToCharBuff(buf, buf_encoded, static_cast<DWORD>(NAMELENGTH));//uncomment
        AnsiToOemBuff(buf, buf_encoded, static_cast<DWORD>(NAMELENGTH));
        // заменить последнюю запись в таблице 10 лучших результатов
        strcpy_s(ttop10[9].name, buf); //buf_encoded
        ttop10[9].date = date;
